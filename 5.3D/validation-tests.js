...
async function run() {
    ...
    // ---- T21 LENGTH: author exceeding maximum (100 chars) ----
    {
        const body = makeValidBook(`b${Date.now() + 15}`);
        body.author = "x".repeat(101);
        await test({
            id: "T21",
            name: "Author exceeding maximum length (100) rejected",
            method: "POST",
            path: createPath,
            expected: 400,
            body,
    ...
    // ---- T22 LENGTH: genre exceeding maximum (50 chars) ----
    {
        const body = makeValidBook(`b${Date.now() + 16}`);
        body.genre = "x".repeat(51);
        await test({
            id: "T22",
            name: "Genre exceeding maximum length (50) rejected",
            method: "POST",
            path: createPath,
            expected: 400,
            body,
    ...
    // ---- T23 LENGTH: summary exceeding maximum (1000 chars) ----
    {
        const body = makeValidBook(`b${Date.now() + 17}`);
        body.summary = "x".repeat(1001);
        await test({
            id: "T23",
            name: "Summary exceeding maximum length (1000) rejected",
            method: "POST",
            path: createPath,
            expected: 400,
            body,
